...
func (gm *gameManager) StartGame(clients []*Client, maxPlayers uint8, seed int64) *GameInstance {
	gm.mu.Lock()
	id := gm.nextID
	gm.nextID++
	gm.mu.Unlock()
...
func (gm *gameManager) GameForClient(client *Client) *GameInstance {
	if client == nil {
		return nil
	}
...
func (gm *gameManager) RemoveGame(id uint64) {
	gm.mu.Lock()
	delete(gm.games, id)
	gm.mu.Unlock()
}
